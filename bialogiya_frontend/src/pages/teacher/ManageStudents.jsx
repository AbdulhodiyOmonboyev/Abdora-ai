...
export default function ManageStudents() {
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: ({ data }) => {
      ...
      if (selectedStudent?.id === data.data.id) setSelectedStudent(data.data);
    },
    onError: (e) => toast.error(e.response?.data?.message || 'Xato'),
  });
  ...
  const freezeMutation = useMutation({
    ...
    onSuccess: ({ data }) => {
      qc.invalidateQueries({ queryKey: ['my-students'] });
      setSelectedStudent(s => s ? { ...s, isFrozen: data.data.isFrozen } : s);
      toast.success(data.data.isFrozen ? 'O\'quvchi muzlatildi' : 'O\'quvchi qayta faollashtirildi');
    },
    onError: (e) => toast.error(e.response?.data?.message || 'Holatni o\'zgartirib bo\'lmadi'),
  });
            ...
            <motion.div initial={{ scale: 0.95, y: 10 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95 }}
              ...
              <div className="flex items-center justify-between mb-5">
                ...
                <div className="flex items-center gap-1">
                  <button onClick={() => { setEditingStudent(selectedStudent.id); setEditForm({ name: selectedStudent.name, phone: selectedStudent.phone || '', groupId: selectedStudent.group?.id || '' }); setSelectedStudent(null); }}
                    className="btn-ghost p-1.5 rounded-lg text-primary" title="Tahrirlash">
                    <Pencil size={15} />
                  </button>
                  <button onClick={() => { deleteMutation.mutate(selectedStudent.id); }}
              ...
              <button onClick={() => freezeMutation.mutate(selectedStudent.id)} disabled={freezeMutation.isPending}
                ...
                <Snowflake size={15} /> {selectedStudent.isFrozen ? 'Muzlatishni bekor qilish' : 'O\'quvchini muzlatish'}
              </button>
