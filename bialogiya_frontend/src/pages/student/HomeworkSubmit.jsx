...
export default function HomeworkSubmit() {
  ...
  const submitMutation = useMutation({
    ...
    onSuccess: () => { toast.success('Uy vazifasi topshirildi!'); navigate('/student/homework'); },
    onError: (err) => toast.error(err.response?.data?.message || 'Topshirishda xatolik'),
  });

  const handleSubmit = () => {
    if (!answer.trim() && files.length === 0) return toast.error('Javob yozing yoki fayl yuklang');
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-xl font-bold text-gray-800 dark:text-white">Uy vazifasini topshirish</h1>
      </div>

      <div className="card space-y-5">
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Sizning javobingiz</label>
          <textarea
            value={answer}
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Fayl yuklash (ixtiyoriy)</label>
          <label className="border-2 border-dashed border-gray-200 dark:border-gray-700 rounded-xl p-6 text-center cursor-pointer hover:border-primary/50 hover:bg-primary/5 transition-all block">
            ...
            <span className="text-sm text-gray-500">PDF, rasm yoki hujjat yuklash uchun bosing</span>
        ...
        <button onClick={handleSubmit} disabled={submitMutation.isPending}
          ...
          {submitMutation.isPending ? 'Topshirilmoqda...' : (<><Send size={16} /> Uy vazifasini topshirish</>)}
        </button>
      </div>
    </div>
  );
