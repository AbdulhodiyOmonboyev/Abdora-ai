...
export default function ManagerPayments() {
  ...
  const { data: groups = [] } = useQuery({
    queryKey: ['manager-groups-for-payments'],
    queryFn: () => api.get('/groups/all').then(r => {
      ...
      <div className="mb-5 flex flex-col gap-3 sm:flex-row">
        <div className="flex-1">
          ...
          <select id="pay-group" value={groupId} onChange={e => setGroupId(e.target.value)}
            ...
            {groups?.map(g => (
              <option key={g.id} value={g.id}>{g.name} ({g.students?.length || g._count?.students || 0} o'quvchi)</option>
            ))}
          </select>
        </div>
