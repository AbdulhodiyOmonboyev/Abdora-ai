      ...
      <div className="flex flex-col gap-4">
        <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
          <div className="max-w-full xl:max-w-[65%]">
            <div className="text-lg font-bold text-gray-800 dark:text-white">Mavzu Yaratuvchi</div>
        ...
        <div className="grid gap-4 grid-cols-1 lg:grid-cols-[minmax(0,320px)_1fr]">
          <section className="space-y-4 rounded-3xl border border-divider bg-secondary-background p-4 min-w-0">
            <div className="flex items-center justify-between gap-2">
              <div className="font-semibold text-gray-800 dark:text-white">Tayyor va saqlangan mavzular</div>
              <span className="rounded-full bg-primary/10 px-3 py-1 text-xs font-semibold text-primary">Jonli</span>
            </div>
                ...
                >
                  <div className="flex flex-col gap-3">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      {theme.type === 'custom' && <span className="text-[11px] font-semibold uppercase tracking-[0.18em] text-primary">Maxsus</span>}
                    </div>
            ...
            <div className="space-y-3 pt-2">
              <label className="block text-sm font-medium text-gray-600 dark:text-gray-300">Maxsus mavzu nomi</label>
              <input
                value={themeName}
            ...
            <div className="space-y-3 pt-2 border-t border-[var(--divider)]">
              <label className="block text-sm font-medium text-gray-600 dark:text-gray-300">Mavzu JSON faylini import qilish</label>
              <textarea
                value={importJson}
                onChange={(event) => setImportJson(event.target.value)}
                rows={5}
                className="input-field resize-none"
                placeholder="Eksport qilingan mavzu JSON kodini shu yerga joylashtiring"
              />
          ...
          <section className="space-y-4 rounded-3xl border border-[var(--divider)] bg-[var(--secondary-background)] p-4 min-w-0">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <div className="font-semibold text-gray-800 dark:text-white">Maxsus ranglar</div>
                <p className="text-sm text-gray-500 dark:text-gray-400">Istalgan rangni o'zgartiring va ko'rinishni tezda kuzating.</p>
              </div>
            ...
            <div className="rounded-3xl border border-[var(--divider)] bg-[var(--background)] p-4 shadow-soft transition-all duration-200">
              <div className="flex items-center justify-between pb-4">
                <div className="font-semibold text-gray-800 dark:text-white">Jonli ko'rinish</div>
                <span className="text-xs text-gray-500 dark:text-gray-400">Silliq 200ms yangilanish</span>
              </div>
              <div className="space-y-4">
                <div className="rounded-3xl border border-[var(--divider)] bg-[var(--navbar-background)] p-4 text-[var(--text-primary)] shadow-sm transition-all duration-200" key={previewKey}>
                  <div className="flex items-center justify-between">
                    <div className="text-sm font-semibold">Yuqori panel</div>
                    <button type="button" className="rounded-full border border-white/20 bg-white/10 px-3 py-1 text-xs text-white/90">Menyu</button>
                  </div>
                </div>
                <div className="grid gap-4 lg:grid-cols-[240px_1fr]">
                  <aside className="rounded-3xl border border-[var(--divider)] bg-[var(--sidebar-background)] p-4 text-[var(--text-primary)] shadow-sm transition-all duration-200">
                    <div className="mb-4 flex items-center gap-2 text-sm font-semibold">Yon panel</div>
                  ...
                  <div className="space-y-4">
                    <div className="rounded-3xl border border-[var(--divider)] bg-[var(--card)] p-4 text-[var(--text-primary)] shadow-sm transition-all duration-200">
                      <div className="flex items-center justify-between gap-2">
                        <div className="text-sm font-semibold">Karta</div>
                        <span className="rounded-full bg-[var(--secondary-background)] px-2 py-1 text-xs font-semibold text-[var(--text-secondary)]">Karta</span>
                      </div>
                      <p className="mt-3 text-sm text-[var(--text-secondary)]">Bu karta doim sahifa foniga qarama-qarshi ko'rinadi.</p>
                    </div>
                    <div className="flex flex-wrap gap-3">
                      <button className="btn-primary">Asosiy</button>
                      <button className="btn-secondary">Ikkinchi darajali</button>
                      <button className="btn-ghost">Shaffof</button>
                    </div>
                    <div className="space-y-3">
                      <input className="input-field" placeholder="Kiritish maydoni" value="" readOnly />
                      <div className="h-12 rounded-2xl border border-[var(--divider)] bg-[var(--input-bg)] px-4 py-3 text-sm text-[var(--text-secondary)]">Ko'rinish uchun kiritish</div>
                    </div>
                  </div>
                </div>
              </div>
