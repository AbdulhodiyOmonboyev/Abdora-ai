...
const branchFilter = (user) => {
  if (user.role === 'reception') {
    return { OR: [{ receptionId: user.userId }, ...(user.branchId ? [{ id: user.branchId }] : [])] };
  }
  if (user.role === 'manager') {
    return { OR: [{ managerId: user.userId }, ...(user.branchId ? [{ id: user.branchId }] : [])] };
  }
  return null;
};
...
const updateExpense = async (req, res, next) => {
  try {
    ...
    const ownBranchIds = await getOwnBranchIds(req.user);
    if (ownBranchIds && !existing.branchId) return error(res, 'Forbidden', 403);
    if (ownBranchIds && !ownBranchIds.includes(existing.branchId)) return error(res, 'Forbidden', 403);
    if (ownBranchIds && branchId !== undefined && (!branchId || !ownBranchIds.includes(branchId))) return error(res, 'Forbidden', 403);
...
const deleteExpense = async (req, res, next) => {
  try {
    ...
    if (!existing) return error(res, 'Xarajat topilmadi', 404);
    const ownBranchIds = await getOwnBranchIds(req.user);
    if (ownBranchIds && (!existing.branchId || !ownBranchIds.includes(existing.branchId))) return error(res, 'Forbidden', 403);
...
const getFinancialAdvice = async (req, res, next) => {
  try {
    ...
    const cacheKey = `finance-advice:${req.user.userId}:${req.query.branchId || 'all-own-branches'}:${month}`;
